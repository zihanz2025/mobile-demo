...
export function InteractiveMapAmap({
  ...
  const uniqueTypes = useMemo(() => {
    ...
    const ordered = Object.keys(typeColors).filter((t) => present.has(t));
    const extras = Array.from(present).filter((t) => !ordered.includes(t));
    return [...ordered, ...extras];
  }, [locations, typeColors]);

  const [typeFilters, setTypeFilters] = useState<Set<string | undefined>>(new Set());
...
useEffect(() => {
  ...
  const filteredLocations = useMemo(() => {
    return locations.filter((loc) => !loc.type || typeFilters.has(loc.type));
  ...
  useEffect(() => {
    ...
    const loadMap = () => {
      ...
      const newMarkers = renderMarkers(mapInstance, filteredLocations);
  ...
  // === Render markers ===
 const renderMarkers = (mapInstance: any, locs: MapLocation[]) => {
  const AMap = window.AMap;
  if (!AMap) return [];
  ...
  const newMarkers = locs
    ...
    .map((loc) => {
      const color = typeColors[loc.type || "未知"] || "#3b82f6";

      // 🧤 Safe fallback if Pixel is not ready
      const offset = hasPixel ? new AMap.Pixel(-10, -10) : [0, 0];
  ...
  return (
    <div className="relative w-full">
      ...
      {showLegend && uniqueTypes.length > 0 && (
        <div className="absolute top-3 right-3 bg-white/95 rounded-lg shadow-lg pointer-events-auto max-w-[220px] z-[999]">
          <div className="flex items-center justify-between p-2 border-b">
            <button
              onClick={() => setLegendExpanded(!legendExpanded)}
              className="text-gray-500 hover:text-gray-700"
            >
              {legendExpanded ? <X className="w-3.5 h-3.5" /> : <Filter className="w-3.5 h-3.5" />}
            </button>
          </div>
    ...
    <label className="flex items-center gap-2 cursor-pointer">
      <Checkbox
        checked={typeFilters.size === uniqueTypes.length}
        indeterminate={
          typeFilters.size > 0 && typeFilters.size < uniqueTypes.length
        }
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5">
                    <span
                      ...
                      style={{ background: typeColors[type || "未知"] || "#3b82f6" }}
                    />
            ...
            <div className="pr-8">
              <div className="flex items-start gap-2.5 mb-2.5">
                <div
                  ...
                  style={{
                    background: typeColors[selectedLocation.type || "未知"] || "#3b82f6",
                  }}
                >
                  <MapPin className="w-3.5 h-3.5" />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-medium mb-1 text-sm">{selectedLocation.name}</h4>
